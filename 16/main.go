...
func castBeamAndCalculateEnergized(grid map[Position]Mirror, s_beam Beam, width, height int, reverse_decode map[Mirror]rune) int {
	working_slice := []Beam{s_beam}
	beam_set := BeamSet{
...
func processWorker(id int, work <-chan Beam, resultChan chan<- int, wg *sync.WaitGroup,
	grid map[Position]Mirror, width, height int, reverse_decode map[Mirror]rune) {
	defer wg.Done()
	for w := range work {
		//fmt.Printf("Worker=%v calculating %v %v\n", id, w.spring, w.config)
		//result := processMirrorImage(w, false)
		result := castBeamAndCalculateEnergized(grid, w, width, height, reverse_decode)
		resultChan <- result
...
func main() {
	...
	workChan := make(chan Beam, channel_length)
	...
	for i := 0; i < numWorkers; i++ {
		wg.Add(1)
		go processWorker(i, workChan, resultChan, &wg, grid, width, height, reverse_decode)
	}
	...
	for x := 0; x < width; x++ {
		for y := 0; y < height; y++ {
			switch x {
			case 0:
				b := Beam{Position{x, y}, RIGHT}
				workChan <- b
			case width - 1:
				b := Beam{Position{x, y}, LEFT}
				workChan <- b
			}
			switch y {
			case 0:
				b := Beam{Position{x, y}, DOWN}
				workChan <- b
			case height - 1:
				b := Beam{Position{x, y}, UP}
				workChan <- b
			}
		}
	}
