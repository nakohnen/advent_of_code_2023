...
func cleanOpenSet(set []WalkCost) []WalkCost {
    result := []WalkCost{}
    ...
    for _, wc := range set {
        ...
        if !found {
            ...
            result = append(result, WalkCost{wc.pos, min_cost, wc.direction})
            is_present[wc] = true
        }
    }
    return result
...
func walkGrid(grid map[Position]int, width, height int, start Position, target Position) int {
	open_set := []WalkCost{}
	open_set = append(open_set, WalkCost{start, 0, HORIZONTAL})
	open_set = append(open_set, WalkCost{start, 0, VERTICAL})
	...
	for len(open_set) > 0 {
        if len(open_set) > 5000 {
            open_set = cleanOpenSet(open_set)
        }
		sort.Slice(open_set, func(i, j int) bool {
			a := open_set[i]
			b := open_set[j]
            d_a2 := abs(a.pos.x - target.x) + abs(a.pos.y - target.y)
            d_b2 := abs(b.pos.x - target.x) + abs(b.pos.y - target.y)
            d_a := math.Sqrt(float64(d_a2))
            d_b := math.Sqrt(float64(d_b2))
            /*if d_a == d_b {
                return a.cost < b.cost
            }*/
			return d_a + float64(a.cost) < d_b + float64(b.cost)
		})
		if debug {
            if len(open_set) > 6 {
		...
		if current.direction == VERTICAL {
			candidates = append(candidates, Vector{Position{current.pos.x - 1, current.pos.y}, HORIZONTAL})
			candidates = append(candidates, Vector{Position{current.pos.x - 2, current.pos.y}, HORIZONTAL})
			candidates = append(candidates, Vector{Position{current.pos.x - 3, current.pos.y}, HORIZONTAL})
			candidates = append(candidates, Vector{Position{current.pos.x + 1, current.pos.y}, HORIZONTAL})
			candidates = append(candidates, Vector{Position{current.pos.x + 2, current.pos.y}, HORIZONTAL})
			candidates = append(candidates, Vector{Position{current.pos.x + 3, current.pos.y}, HORIZONTAL})
		} else if current.direction == HORIZONTAL {
			candidates = append(candidates, Vector{Position{current.pos.x, current.pos.y - 1}, VERTICAL})
			candidates = append(candidates, Vector{Position{current.pos.x, current.pos.y - 2}, VERTICAL})
			candidates = append(candidates, Vector{Position{current.pos.x, current.pos.y - 3}, VERTICAL})
			candidates = append(candidates, Vector{Position{current.pos.x, current.pos.y + 1}, VERTICAL})
			candidates = append(candidates, Vector{Position{current.pos.x, current.pos.y + 2}, VERTICAL})
			candidates = append(candidates, Vector{Position{current.pos.x, current.pos.y + 3}, VERTICAL})
		}
		for _, c := range candidates {
			...
			if pos.x >= 0 && pos.y >= 0 && pos.x < width && pos.y < height {
				...
				if new_cost <= cost_map[c] {
					wc := WalkCost{c.pos, new_cost, c.dir}
