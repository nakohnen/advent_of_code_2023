...
var cardStrength string = "AKQT98765432J"
...
func compareHands(hand string, other string, strength string, typeStrength map[string]int) int {
	...
	if count_type_1[len(strength)-1] > 0 {
		max_val := 0
		for _, c := range count_type_1[:len(strength)-1] {
			if c > max_val {
				max_val = c
			}
		}
		for i := 0; i < len(count_type_1)-1; i++ {
			if count_type_1[i] == max_val {
				count_type_1[i] += count_type_1[len(strength)-1]
				count_type_1[len(strength)-1] = 0
				break
			}
		}
	}

	if count_type_2[len(strength)-1] > 0 {
		max_val := 0
		for _, c := range count_type_2[:len(strength)-1] {
			if c > max_val {
				max_val = c
			}
		}
		for i := 0; i < len(count_type_2)-1; i++ {
			if count_type_2[i] == max_val {
				count_type_2[i] += count_type_2[len(strength)-1]
				count_type_2[len(strength)-1] = 0
				break
			}
		}
	}

	fmt.Printf("%v Hand 1: %v\n", hand, count_type_1)
	fmt.Printf("%v Hand 2: %v\n", other, count_type_2)
